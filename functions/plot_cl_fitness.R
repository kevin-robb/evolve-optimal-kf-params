...
if (length(args)==0) {
  stop("Must supply filename in format 'ec_2021-03-08-18-23-03'", call.=FALSE)
...
if (tolower(args[2]) == "true") {
  plot_ec_fitness(filename=args[1],png=TRUE)
} else {
  plot_ec_fitness(filename=args[1],png=FALSE)
}
